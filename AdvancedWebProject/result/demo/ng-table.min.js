!function (a, b) {
  "use strict";
  return"function" == typeof define && define.amd ? (define(["../bower_components/angular/angular"], function (a) {
    return b(a)
  }), void 0) : b(a)
    ...
    var d = function (a) {
      ...
      this.data = [], this.parameters = function (b, c) {
        ...
        if (d = 11, i = [], h = Math.ceil(b / c), h > 1) {
          i.push({type: "prev", number: Math.max(1, a - 1), active: a > 1}), i.push({type: "first", number: 1, active: a > 1,current:a===1}), f = Math.round((d - 5) / 2), g = Math.max(2, a - f), e = Math.min(h - 1, a + 2 * f - (a - g)), g = Math.max(2, g - (2 * f - (e - g)));
          for (var j = g; e >= j;)j === g && 2 !== j || j === e && j !== h - 1 ? i.push({type: "more", active: !1}) : i.push({type: "page", number: j, active: a !== j,current:a===j}), j++;
          i.push({type: "last", number: h, active: a !== h,current:a===h }), i.push({type: "next", number: Math.min(h, a + 1), active: h > a})
        }
        return i
      }, this.url = function (b) {
        b = b || !1;
  ...
  return b.directive("ngTable", ["$compile", "$q", "$parse", function (b, d, e) {
    return{restrict: "A", priority: 1001, scope: !0, controller: c, compile: function (c) {
      ...
      return a.forEach(a.element(c.find("tr")), function (b) {
        ...
        if (c.$loading = !1, c.$columns = d, c.$watch(g.ngTable, function (b) {
          ...
          f.find("thead").remove(); if(f.find('colgroup')[0]){f.find('colgroup').after(i),f.addClass("ng-table").after(j)}else{f.addClass("ng-table").prepend(i).after(j)}; b(i)(c), b(j)(c)
        }
      }) : void 0
    }}
    ...
    a.put("ng-table/total.html",'<span>asdasdasd</span>>'),
        a.put("ng-table/filters/select-multiple.html", '<select ng-options="data.id as data.title for data in column.data" multiple ng-multiple="true" ng-model="params.filter()[name]" ng-show="filter==\'select-multiple\'" class="filter filter-select-multiple form-control" name="{{column.filterName}}"> </select>'),
        ...
        a.put("ng-table/pager.html", '<div class="ng-cloak ng-table-pager pageRight"><div class="pageTotal"><span>总共{{params.total()}}条数据</span></div> <ul class="pagination ng-table-pagination" style="height: 34px"> <li ng-class="{\'disabled\': !page.active,active_page:page.current}" ng-repeat="page in pages" ng-switch="page.type"> <a ng-switch-when="prev" class="page_prev" ng-click="params.page(page.number)" href="">&laquo;</a> <a ng-switch-when="first" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="page" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a> <a ng-switch-when="last" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="next" ng-click="params.page(page.number)" href="">&raquo;</a> </li> </ul> </div> ')
  }]), b
});